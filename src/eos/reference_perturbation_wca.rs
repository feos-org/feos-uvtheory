...
impl<D: DualNum<f64>> HelmholtzEnergyDual<D> for ReferencePerturbationWCA {
    ...
    fn helmholtz_energy(&self, state: &StateHD<D>) -> D {
        ...
        let eta_a = packing_fraction_a(p, &d, eta, state.temperature);
        let eta_b = packing_fraction_b(p, &d, eta, state.temperature);
        let mut a = D::zero();
        ...
        for i in 0..n {
            for j in 0..n {
                ...
                a += x[i] * x[j] * (-eta_a[[i, j]] * 0.5 + 1.0) / (-eta_a[[i, j]] + 1.0).powi(3)
...
mod test {
    use super::*;
    use crate::parameters::utils::test_parameters;
